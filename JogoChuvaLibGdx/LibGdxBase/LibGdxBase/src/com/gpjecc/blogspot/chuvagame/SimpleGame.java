...
public class SimpleGame extends ApplicationAdapter {
   ...
   void carregaOsave() throws FileNotFoundException {
      File arquivoEntrada = new File("assets/save.txt");
      Scanner scArquivo = new Scanner(arquivoEntrada);

      while (scArquivo.hasNextLine()) {
         String posicoes = scArquivo.nextLine();
         String[] posicao = posicoes.split(" ");

         for (int i = 0; i < posicoes.length(); i++) {
            Rectangle raindrop = new Rectangle();
            raindrop.x = Float.valueOf(posicao[0]);
            raindrop.y = Float.valueOf(posicao[1]);
            raindrop.width = 64;
            raindrop.height = 64;
   ...
   void salvaOJogo() throws FileNotFoundException {

      Iterator<Rectangle> iter = raindrops.iterator();
      FileOutputStream out = new FileOutputStream("assets/save.txt");
      PrintStream ps = new PrintStream(out);

      while (iter.hasNext()) {
         Rectangle raindrop = iter.next();
         ps.println(raindrop.x + " " + raindrop.y);
      }
